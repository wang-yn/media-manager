...
def preview_rename(item: MediaItem) -> dict[str, object]:
...
def apply_rename(item: MediaItem) -> dict[str, object]:
...
def _changes(item: MediaItem) -> list[dict[str, str]]:
    video = Path(item.path)
    target_video = _target_video(item)
    changes = [{"from": str(video), "to": str(target_video)}]
